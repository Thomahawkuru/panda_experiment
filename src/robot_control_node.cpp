...
static const std::string APP_DIRECTORY_NAME = ".panda_experiment";
...
int main(int argc, char **argv)
{
  ...
  // read JSON files from ~/.panda_experiment
  ...
  if (fs::is_directory(home))
  {
    ...
    if (!fs::exists(app_directory) && !fs::is_directory(app_directory))
    {
      ...
      // Create .panda_experiment directory
      std::string path(getenv("HOME"));
      path += "/.panda_experiment";
